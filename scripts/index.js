...
app.post("/equipos", upload.single("logo"), async (req, res) => {
    ...
    try {
        ...
        //CREAR EQUIPO
        const tx = await equiposContract.crearEquipo(idEquipo, nombre, logoIPFS, nftId);
        await tx.wait();

        //ACUÑAR/MINAR NFT PARA EL EQUIPO [CAMBIO MAS RECIENTE: 06-12-2024]
        const nftTx = await nftContract.mintColeccionable(wallet.address, nftMetaData, idEquipo) //EDITAR
        //const nftTx = await nftContract.mintColeccionable(wallet.address, nftMetaData);
        await nftTx.wait();

        //CAMBIO MAS RECIENTE: 06-12-2024
        res.json({ message: "Equipo creado con éxito y NFT minado", nombre, nftId, idEquipo, logoIPFS, txHash: tx.hash, nftTxHash: nftTx.hash });
...
app.get("/equipos/:idEquipo", async (req, res) => {
    ...
    try {
        ...
        if (error.code === "CALL_EXCEPTION") {
            res.status(404).json({ error: "El equipo no existe o no se pudo encontrar" });
        } else {
            res.status(500).json({ error: error.message });
        }
        //res.status(500).json({ error: error.message });
...
app.delete("/equipos/:idEquipo", async (req, res) => {
    ...
    try {
        ...
        res.json({ message: "Equipo y jugadores eliminados con éxito", txHash: tx.hash });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
app.post("/jugadores", upload.single("imagen"), async (req, res) => {
    const { idJugador, nombre, nickname, rol, nacionalidad, equipoId, nftMetaData, nftId } = req.body; //[CAMBIO MAS RECIENTE - 06-12-2024]
    ...
    try {
        ...
        //REGISTRAR JUGADOR
        const tx = await jugadoresContract.registrarJugador(idJugador, nombre, nickname, rol, nacionalidad, imagenIPFS, equipoId, nftId);
        await tx.wait();
        ...
        if (equipoId) {
            const txEquipo = await equiposContract.agregarJugador(equipoId, idJugador); //ID DE EQUIPO Y ID DE JUGADOR
            await txEquipo.wait();
        }
        res.json({ message: "Jugador registrado con éxito y NFT minado", imagenIPFS, txHash: tx.hash, nftTxHash: nftTx.hash });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
